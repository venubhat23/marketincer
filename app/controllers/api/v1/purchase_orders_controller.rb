...
module Api
  module V1
    class PurchaseOrdersController < ApplicationController
      ...
      def create
        purchase_order = @current_user.purchase_orders.build(purchase_order_params)
      ...
      def purchase_order_params
        params.require(:purchase_order).permit(:company_name, :gst_number, :phone_number, :address, 
                                               :company_website, :job_title, :work_email, :gst_percentage, 
                                               :status, :order_number, line_items: [:description, :quantity, :unit_price])
      end
    end
  end
end
